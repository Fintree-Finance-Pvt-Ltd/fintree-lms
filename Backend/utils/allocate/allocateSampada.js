...
/**
 * Allocate payments for Sampada loans.
 * Knock off interest first, then principal (oldest dues first).
 */
const allocateSampada = async (lan, payment) => {
  ...
  if (remaining > 0) {
    ...
    console.log(
      `✅ Excess payment parked for Sampada LAN ${lan}`
    );
  ...
  if (pending.count === 0) {
    ...
    console.log(
      `✅ Loan status updated to Fully Paid for Sampada LAN ${lan}`
    );
  }
...
module.exports = allocateSampada;