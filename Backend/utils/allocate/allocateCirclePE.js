...
/**
 * Allocate payments for CirclePE loans.
 * Knock off interest first, then principal (oldest dues first).
 */
const allocateCirclePE = async (lan, payment) => {
  ...
  const emiTable = "manual_rps_circlepe";
  const loanTable = "loan_booking_circle_pe";
  ...
  if (remaining > 0) {
    ...
    console.log(`✅ Excess payment parked for CirclePE LAN ${lan}`);
    remaining = 0;
  }
  ...
  if (pending.count === 0) {
    ...
    console.log(`✅ Loan status updated to Fully Paid for CirclePE LAN ${lan}`);
  }
};

module.exports = allocateCirclePE;
