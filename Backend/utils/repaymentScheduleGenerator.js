...
const generateRepaymentScheduleGQNonFSF = async (
  ...
  try {
    console.log(`\n🚀 Generating GQ NON-FSF RPS for LAN: ${lan}`);
    console.log(
      `📝 Inputs → ApprovedAmount: ₹${approvedAmount}, InterestRate: ${interestRate}%, Tenure: ${tenure}, DisbursementDate: ${disbursementDate}, EMI Date: ${emiDate}, SubventionAmount: ₹${subventionAmount}, Product: ${product}, Lender: ${lender}, AdvanceEMIs: ${no_of_advance_emis}`
    );

    const annualRate = Number(interestRate) / 100;
    let remainingPrincipal = Number(approvedAmount);

    const isZeroInterest = annualRate === 0;
    const emiPrincipal = Math.round(remainingPrincipal / Number(tenure));
    let emiInterest = isZeroInterest ? 0 : Math.ceil((remainingPrincipal * annualRate) / Number(tenure));
    let emiTotal = emiPrincipal + emiInterest;
    ...
    for (let i = 1; i <= tenure; i++) {
      ...
      if (i === tenure) {
        // last row takes whatever principal is left
        principal = remainingPrincipal;
        emiTotal = principal + interest;
      }
      ...
      let dueDate;
      if (no_of_advance_emis > 0 && i === 1) {
        dueDate = new Date(disbursementDate); // first EMI on disbursement
      } else {
        const offset = no_of_advance_emis > 0 ? i - 2 : i - 1;
        dueDate = getFirstEmiDate(disbursementDate, emiDate, lender, product, offset);
      }
      ...
      const principalR = r2(principal);
      const interestR  = r2(interest);
      const emiR       = r2(emiTotal);

      // --- YOUR REQUIRED MAPPINGS ---
      // Remaining Principal = Principal
      // Remaining Interest  = Interest
      // Remaining Amount    = Remaining Principal + Remaining Interest
      const remainingPrincipalR = principalR;
      const remainingInterestR  = interestR;
      const remainingAmountR    = r2(remainingPrincipalR + remainingInterestR); // equals emiR if consistent
      ...
      rpsData.push([
        ...
        emiR,                      // emi
        interestR,                // interest
        principalR,               // principal
        remainingPrincipalR,      // remaining_principal (== principal)
        remainingInterestR,       // remaining_interest  (== interest)
        remainingAmountR,         // remaining_emi / amount (== principal + interest)
        "Pending"
      ]);

      remainingPrincipal = r2(remainingPrincipal - principalR);
    }
