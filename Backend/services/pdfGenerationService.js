...
function formatCcbPercentage(value) {
  ...
  return Number.isFinite(percentage) ? percentage.toFixed(2) : String(value);
}
...
async function getClaimCureBuddyLoanData(lan) {
  ...
  const processingFeePercentage =
    summary.processing_fee_percentage ?? calculatedProcessingFeePercentage;

  const totalFees = Number(summary.total_fees_and_charges || processingFee);
  ...
  return {
    AGREEMENT_REFERENCE: summary.agreement_reference || `AGR-${lan}`,
    ...
    DOCUMENT_GENERATED_AT: `${dayjs().format("DD/MM/YYYY HH:mm:ss")} IST`,
    ...
    LSP_NAME_HI: extraValue("LSP_NAME_HI", process.env.CCB_LSP_NAME_HI || ""),
    ...
    SANCTIONED_LOAN_AMOUNT_IN_WORDS: ccbAmountInWords(loanAmount),

    SANCTIONED_LOAN_AMOUNT_IN_WORDS_HI:
      summary.sanctioned_amount_in_words_hi || "",

    NET_DISBURSED_AMOUNT: formatCcbAmount(netDisbursedAmount),

    NET_DISBURSED_AMOUNT_IN_WORDS: ccbAmountInWords(netDisbursedAmount),
    ...
    LOAN_TERM:
      ...
      (booking.loan_tenure ? `${booking.loan_tenure} Days` : ""),

    INTEREST_RATE: formatCcbPercentage(booking.interest_rate),

    INTEREST_RATE_TYPE: extraValue("INTEREST_RATE_TYPE", "Fixed"),

    APR: formatCcbPercentage(summary.apr),

    TOTAL_INTEREST_AMOUNT: formatCcbAmount(totalInterest),

    TOTAL_REPAYMENT_AMOUNT: formatCcbAmount(totalRepaymentAmount),

    DISBURSAL_PERCENTAGE: extraValue("DISBURSAL_PERCENTAGE", "100% upfront"),

    DISBURSAL_CLAUSE_NUMBER: extraValue("DISBURSAL_CLAUSE_NUMBER", "6"),

    DISBURSAL_SCHEDULE_DESCRIPTION: extraValue(
      "DISBURSAL_SCHEDULE_DESCRIPTION",
      "100% upfront after completion of conditions precedent",
    ),

    DISBURSEMENT_DATE: formatCcbDate(summary.disbursement_date),

    DISBURSEMENT_REQUEST_DATE: formatCcbDate(summary.disbursement_request_date),

    DISBURSEMENT_MODE: summary.disbursement_mode || "",

    DISBURSEMENT_TRANSACTION_REFERENCE:
      summary.disbursement_transaction_reference || "",

    PAYEE_NAME: booking.customer_name_as_per_bank || borrowerName,
    ...
    INSTALLMENT_AMOUNT: formatCcbAmount(installmentAmount),
    ...
    INSTALLMENT_DUE_DATE_DESCRIPTION: repaymentDate
      ? `On or before ${repaymentDate}`
      : "",
    ...
    PROCESSING_FEE_AMOUNT: formatCcbAmount(processingFee),

    PROCESSING_FEE_PERCENTAGE: formatCcbPercentage(processingFeePercentage),

    PROCESSING_FEE_THIRD_PARTY_FREQUENCY: extraValue(
      "PROCESSING_FEE_THIRD_PARTY_FREQUENCY",
      "NA",
    ),

    PROCESSING_FEE_THIRD_PARTY_AMOUNT: extraValue(
      "PROCESSING_FEE_THIRD_PARTY_AMOUNT",
      "0.00",
    ),

    INSURANCE_FEE_FREQUENCY: extraValue("INSURANCE_FEE_FREQUENCY", "NA"),

    INSURANCE_FEE_AMOUNT: formatCcbAmount(summary.insurance_fee_amount || 0),

    INSURANCE_THIRD_PARTY_FREQUENCY: extraValue(
      "INSURANCE_THIRD_PARTY_FREQUENCY",
      "NA",
    ),

    INSURANCE_THIRD_PARTY_AMOUNT: extraValue(
      "INSURANCE_THIRD_PARTY_AMOUNT",
      "0.00",
    ),

    STAMP_DUTY_FREQUENCY: extraValue("STAMP_DUTY_FREQUENCY", "One-time"),

    STAMP_DUTY_AMOUNT: formatCcbAmount(summary.stamp_duty_amount || 0),

    STAMP_DUTY_THIRD_PARTY_FREQUENCY: extraValue(
      "STAMP_DUTY_THIRD_PARTY_FREQUENCY",
      "NA",
    ),

    STAMP_DUTY_THIRD_PARTY_AMOUNT: extraValue(
      "STAMP_DUTY_THIRD_PARTY_AMOUNT",
      "0.00",
    ),

    TOTAL_FEES_AND_CHARGES: formatCcbAmount(totalFees),

    LENDER_CHARGES: formatCcbAmount(totalFees),

    THIRD_PARTY_CHARGES: formatCcbAmount(summary.third_party_charges || 0),
    ...
    RECOVERY_AGENT_CLAUSE_NUMBER: extraValue(
      "RECOVERY_AGENT_CLAUSE_NUMBER",
      "16",
    ),

    GRIEVANCE_CLAUSE_NUMBER: extraValue("GRIEVANCE_CLAUSE_NUMBER", "15"),

    PROMISSORY_NOTE_DATE: extraValue("PROMISSORY_NOTE_DATE", agreementDate),

    WITNESS_NAME: extraValue("WITNESS_NAME"),
    WITNESS_ADDRESS: extraValue("WITNESS_ADDRESS"),

    WITNESS_SIGNATURE_REFERENCE: extraValue("WITNESS_SIGNATURE_REFERENCE"),

    ACCEPTANCE_DATE: extraValue("ACCEPTANCE_DATE", agreementDate),

    DIGITALLY_ACCEPTED_BY: extraValue("DIGITALLY_ACCEPTED_BY", borrowerName),
    ...
    VERNACULAR_EMPLOYEE_NAME: extraValue("VERNACULAR_EMPLOYEE_NAME"),
    VERNACULAR_EMPLOYEE_ADDRESS: extraValue("VERNACULAR_EMPLOYEE_ADDRESS"),

    VERNACULAR_EMPLOYEE_SIGNATURE_REFERENCE: extraValue(
      "VERNACULAR_EMPLOYEE_SIGNATURE_REFERENCE",
    ),

    BRANCH_SIGNATURE_REFERENCE: extraValue("BRANCH_SIGNATURE_REFERENCE"),

    VERNACULAR_DECLARATION_DATE: extraValue(
      "VERNACULAR_DECLARATION_DATE",
      agreementDate,
    ),

    END_USE_UNDERTAKING_DATE: extraValue(
      "END_USE_UNDERTAKING_DATE",
      agreementDate,
    ),

    CONSENT_DATE: extraValue("CONSENT_DATE", agreementDate),
  };
}
...
async function getLoanData(lan) {
  ...
  const loanAmount =
    summary.L_A || summary.LOAN_AMOUNT || summary.FINAL_LIMIT || 0;
...
async function generatePdfFromHtml(html, fileName, options = {}) {
  ...
  if (options.claimCureBuddy) {
    await page.waitForFunction(() => window.__AGREEMENT_READY__ === true, {
      timeout: 120000,
    });
  }
...
exports.generateAgreementPdf = async (lan) => {
  ...
  if (type === "CLAIM_CURE_BUDDY") {
    await db
      .promise()
      .query("CALL sp_create_claim_cure_buddy_loan_summary(?)", [lan]);
    const loanData = await waitForLoanSummary(lan);

    if (!loanData) {
      throw new Error("ClaimCureBuddy booking or summary is not available");
    }
  ...
  if (summaryTable === "motioncorp_loan_summary") {
    ...
    await db.promise().query("CALL sp_generate_motioncorp_rps(?)", [lan]);
