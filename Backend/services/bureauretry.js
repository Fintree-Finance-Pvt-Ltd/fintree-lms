...
router.post(
  ...
  async (req, res) => {
    ...
    const partner =
      PARTNERS[String(partnerKey).toLowerCase()];

    if (!partner) {
      return res.status(400).json({
        status: "FAILED",
        message: `Unknown partner: ${partnerKey}`,
      });
    }

    try {
      const result =
        await retriggerFailedBureauBatch(
          partner.key,
          limit,
        );
      ...
      console.error(
        `bulk retrigger error for ${partnerKey}:`,
        err,
      );
