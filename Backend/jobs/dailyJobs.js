...
//   await db.promise().query("CALL sp_refresh_metric_summaries(?, ?)", [from, to]);
...
cron.schedule(
  ...
  async () => {
    ...
    console.log("⏳ [CRON] sp_refresh_metric_summaries started at:", startTime.toISOString());
    ...
    try {
      ...
      console.log(`📅 [CRON] Date range -> from: ${from}, to: ${to}`);

      await db
        .promise()
        .query("CALL sp_refresh_metric_summaries(?, ?)", [from, to]);

      const endTime = new Date();
      console.log(
        "✅ [CRON] sp_refresh_metric_summaries completed at:",
        endTime.toISOString(),
        "| Duration:",
        `${(endTime - startTime) / 1000}s`
      );
    } catch (error) {
      console.error("❌ [CRON] sp_refresh_metric_summaries failed:", error);
    }
  },
  {
    timezone: "Asia/Kolkata",
