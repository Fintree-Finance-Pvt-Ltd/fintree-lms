...
async function queueToday() {
  ...
  const [r2] = await pool.query(sqlOverdue, [
    SENDER_ID,
    DLT_TEMPLATE_ID_OVERDUE,
  ]);
...
// // -------------------- queue DUE + OVERDUE --------------------
// async function queueToday() {
//   const rpsTable = "manual_rps_dummy";
//   const bookingTable = "loan_bookings_dummy";
...
//     SELECT 'DUE', r.lan, r.due_date, b.mobile_number, b.customer_name,
//       CONCAT(
//         'Dear ', COALESCE(b.customer_name,'Customer'),
//         ', your EMI of Rs.', ROUND(r.emi),
//         ' for Loan A/c ', r.lan,
//         ' is due on ', DATE_FORMAT(r.due_date, '%d %b %Y'),
//         '. Kindly ensure payment on or before the due date to avoid late charges.Regards Fintree Finance Private Ltd.'
//       ),
//       ROUND(r.emi * 100),
//       ?, ?, 'QUEUED'
//     FROM \`${rpsTable}\` r
//     JOIN \`${bookingTable}\` b ON b.lan = r.lan
//     LEFT JOIN sms_outbox o ON o.sms_type='DUE' AND o.lan=r.lan AND o.due_date=r.due_date AND DATE(o.created_at)=CURDATE()
//     WHERE r.due_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 4 DAY)
//       AND (r.status IS NULL OR r.status <> 'Paid')
//       AND o.id IS NULL
//   `;

//   // OVERDUE: send only on T+2 and T+4
//   const sqlOverdue = `
//     INSERT INTO sms_outbox 
//       (sms_type, lan, due_date, mobile, customer_name, message, amount_paise, dlt_header, dlt_template_id, status)
//     SELECT 'OVERDUE', r.lan, r.due_date, b.mobile_number, b.customer_name,
//       CONCAT(
//         'Payment of Rs. ', ROUND(r.emi),
//         ' for Loan A/c ', r.lan,
//         ', due on ', DATE_FORMAT(r.due_date, '%d %b %Y'),
//         ', is still pending. Please clear dues at the earliest.Regards Fintree Finance Private Limited.'
//       ),
//       ROUND(r.emi * 100),
//       ?, ?, 'QUEUED'
//     FROM \`${rpsTable}\` r
//     JOIN \`${bookingTable}\` b ON b.lan = r.lan
//     LEFT JOIN sms_outbox o ON o.sms_type='OVERDUE' AND o.lan=r.lan AND o.due_date=r.due_date AND DATE(o.created_at)=CURDATE()
//     WHERE DATEDIFF(CURDATE(), r.due_date) IN (2,4)
//       AND (r.status IS NULL OR r.status <> 'Paid')
//       AND (r.payment_date IS NULL OR DATE(r.payment_date) > DATE(r.due_date))
//       AND o.id IS NULL
...
// async function runOnce() {
//   const q = await queueToday();
//   const sent = await sendQueued(50);
//   console.log("📨 SMS | queued:", q, "| sent:", sent);
// }

// function initScheduler() {
//   cron.schedule("5 12 * * *", () => runOnce(), { timezone: tz }); // daily queue
//   cron.schedule(
//     "*/10 * * * *",
//     () => sendQueued(50).then((n) => n && console.log("🚚 sent batch:", n)),
//     { timezone: tz }
//   );
//   console.log("⏰ SMS scheduler active (IST).");
// }
