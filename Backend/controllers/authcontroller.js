  ...
  db.query(
    ...
    async (err, results) => {
      ...
      await new Promise((resolve, reject) => {
        db.query(
          "DELETE FROM reset_otps WHERE user_id = ?",
          [userId],
          (err) => {
            if (err) reject(err);
            else resolve();
        ...
        async (err) => {
          ...
          try {
            ...
            res
              .status(500)
              .json({
                message: "OTP generated but mail failed. Check your inbox.",
              });
          }
        },
      );
...
exports.resetPassword = async (req, res) => {
  ...
  if (!email || !otp || !newPassword || newPassword.length < 6) {
    return res
      .status(400)
      .json({
        message: "Valid email, OTP, and password (min 6 chars) required",
      });
  }
    ...
    async (err, results) => {
      ...
      db.query(
        ...
        (err) => {
          ...
          // Mark OTP used
          db.query(
            "UPDATE reset_otps SET used = TRUE WHERE user_id = ? AND otp = ?",
            [userId, otp],
            (err) => {
