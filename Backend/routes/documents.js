...
const checkAndApproveCarePayLoan = async (lan) => {
  ...
  // ABB required:
  // loan > 3L OR score 1-200 OR score exactly 680
  const abbRequired =
    loanAmount > 300000 ||
  ...
  if (bankStatementRequired) {
    requiredDocuments.push("bankStatement");
  }

  const [documentRows] = await db.promise().query(
    `SELECT doc_name, original_name
     FROM loan_documents
     WHERE lan = ?`,
    [cleanLan],
  );
...
router.post(
  ...
  (req, res) => {
    ...
    if (!lan || !req.files?.length) {
      return res.status(400).json({
        error: "LAN and files are required.",
      });
    }
    ...
    const values = req.files.map((file) => [
      ...
      docName,
      file.filename,
      file.originalname?.trim() || file.filename,
      new Date(),
    ]);
  ...
  async (req, res) => {
    try {
      ...
      if (docNames.length !== req.files.length) {
        return res.status(400).json({
          status: "Failed",
          message: "Document names count must match uploaded files count.",
          files_count: req.files.length,
          doc_names_count: docNames.length,
        });
      }

      const values = req.files.map((file, index) => [
        lan.trim(),
        file.filename, // multer generated filename
        file.originalname.trim(), // partner original filename
        docNames[index].trim(), // readable document name
        new Date(),
      ]);
  ...
  try {
    ...
    (allocationRows || []).forEach((a) => {
      ...
      credits.push({
        ...
        other:
          ct !== "principal" && ct !== "interest" ? amt.toFixed(2) : "0.00",
        total: amt.toFixed(2),
        payment_date,
      });
    });
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(16)
      .text("Statement Of Account", 520, 26, { align: "right" });
    ...
    drawPremiumCard(
      ...
      `Rs. ${inr(principalOutstanding)}`,
      true,
    );
    drawPremiumCard(
      doc,
      MARGINS.left + (cardW + gap) * 1,
      cardY1,
      cardW,
      cardH,
      "TOTAL OVERDUE",
      `Rs. ${inr(totalEmiOverdue)}`,
    );
    drawPremiumCard(
      doc,
      MARGINS.left + (cardW + gap) * 2,
      cardY1,
      cardW,
      cardH,
      "SANCTIONED AMOUNT",
    ...
    doc
      .fillColor("#F8FAFC")
      .roundedRect(MARGINS.left, infoPlateY, 725, infoPlateH, 4)
      .fill();
    doc
      .lineWidth(0.5)
      .strokeColor("#E2E8F0")
    ...
    doc.text(
      "ACCOUNT HOLDERS SUMMARY DETAILS",
      MARGINS.left + 10,
      infoPlateY + 7,
    );
    ...
    doc
      .fillColor("#2c55b5")
      .font("Helvetica-Bold")
      .text(`${loan.customer_name || "-"}`, MARGINS.left + 285, infoPlateY + 7);

    doc
      .fillColor("#64748B")
      .font("Helvetica")
      .text(`Loan Account No:`, MARGINS.left + 410, infoPlateY + 7);
    doc
      .fillColor("#2c55b5")
      .font("Helvetica-Bold")
      .text(`${loan.lan || "-"}`, MARGINS.left + 490, infoPlateY + 7);

    doc
      .fillColor("#64748B")
      .font("Helvetica")
      .text(`Partner ID:`, MARGINS.left + 595, infoPlateY + 7);
    doc
      .fillColor("#2c55b5")
      .font("Helvetica-Bold")
      .text(`${partnerLoanId}`, MARGINS.left + 645, infoPlateY + 7);
    doc.restore();
    ...
    for (const r of rows) {
      if (y + rowH > bottomLimitY) {
        ...
        doc
          .lineWidth(0.5)
          .strokeColor("#CBD5E0")
          .rect(MARGINS.left, tableStartY, 725, y - tableStartY)
          .stroke();
        doc.restore();
      ...
      doc
        ...
        .lineTo(MARGINS.left + 725, y + rowH)
        .stroke();
      doc.restore();
