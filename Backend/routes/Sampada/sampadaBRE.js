...
// const extractSampadaBureauFacts = (reportXml) => {
//   if (!reportXml) {
//     return {
//       score: null,
//       enquiries30d: 0,
...
// const evaluateSampadaPolicy = ({ loan, bureauFacts }) => {
//   const reasons = [];
//   const deviations = [];
...
//   if (!kycRows.length) {
//     console.log("No Sampada KYC row found:", lan);
...
//     console.log(`Sampada BRE pending for ${lan}: ${pendingReason}`);
...
//   if (!loanRows.length) {
//     console.log("Sampada loan not found:", lan);
...
//   const bureauFacts = extractSampadaBureauFacts(cibilRows[0].report_xml);

//   const decision = evaluateSampadaPolicy({
//     loan,
//     bureauFacts,
//   });
...
//   console.log(
//     `Sampada BRE completed for ${lan}: ${decision.status} | ${reasonText}`,
//   );
// };
...
const evaluateSampadaBureauScreening = ({ loanPan, bureauFacts }) => {
  const reasons = [];
  const deviations = [];
 ...
 * ===========================================================
 */
const evaluateSampadaPolicy = ({ loan, bureauFacts, amlStatus }) => {
  const reasons = [];
  const deviations = [];
...
//     `
//     SELECT COUNT(*) AS cnt
//     FROM loan_booking_sampada other
//     JOIN loan_booking_sampada self
//       ON self.lan = ?
...
const autoApproveSampadaIfAllVerified = async (lan) => {
  ...
  // console.log("Sampada borrower CIBIL check:", {
  //   lan,
  ...
  console.log("Sampada borrower bureau check:", {
    lan,
  ...
  if (!hasValue(reportXml)) {
    console.log("No usable bureau XML found for Sampada:", {
      lan,
  ...
  try {
    const aml = await screenLoanBooking("sampada", lan);
  ...
  try {
    bureauFacts = extractSampadaBureauFacts(reportXml);
  } catch (error) {
    console.error("Sampada bureau XML parsing failed:", {
      lan,
