...
const source = fs.readFileSync(path.join(__dirname, "../routes/switchMyLoan/switchMyLoanRotues.js"), "utf8");
const start = source.indexOf('router.post("/v1/create"');
const end = source.indexOf('// router.put("/v1/update-details"', start);
...
async function run({ failures = [], rollbackFails = false, duplicateOnRetry = false } = {}) {
  ...
  const db = { promise: () => ({ getConnection: async () => {
    ...
    return {
      ...
      query: async (sql) => {
        if (sql.includes("blacklist_customer")) return [[]];
  ...
  vm.runInNewContext(source.slice(start, end), {
    ...
    verifyApiKey: () => {}, db, RETRYABLE_SEQUENCE_ERROR_CODES: new Set(["ER_LOCK_WAIT_TIMEOUT", "ER_LOCK_DEADLOCK", "ER_CHECKREAD"]),
    normalizeCreateUpdatePayload: () => ({}),
  ...
  await handler({ body: { partner_loan_id: "partner", lenderType: "RAPID-MONEY" } }, res);
...
for (const code of ["ER_LOCK_WAIT_TIMEOUT", "ER_LOCK_DEADLOCK", "ER_CHECKREAD"]) {
  test(`${code} rolls back all writes and releases before retry`, async () => {
    ...
    assert.equal(res.body.is_success, true);
