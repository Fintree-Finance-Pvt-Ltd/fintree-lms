...
const createInitialLoan = () => ({
  login_date: getLocalToday(),
...
const FundifyManualEntry = () => {
  ...
  const [businessSaved, setBusinessSaved] = useState(false); // LAN created when business section saved
  ...
  const otpTimersRef = React.useRef({});

  /* ---- Data ---- */
  const [applicant, setApplicant] = useState(emptyApplicant("BORROWER", 1));
  const [loan, setLoan] = useState(createInitialLoan);
...
useEffect(() => {
  if (!resumeLan) return;
  let active = true;
  fetchResumeBooking(resumeLan, () => active);
  return () => { active = false; };
  ...
  const fetchResumeBooking = async (resumeLanParam) => {
    ...
    try {
      const res = await api.get(
        `/fundify/fundify-manual-entry/${resumeLanParam}`,
      );

      if (!res.data?.success) {
        throw new Error(
          res.data?.message || "Could not load existing loan details",
        );
      }
      ...
      const applicantRows = Array.isArray(applicants) ? applicants : [];

      const kycRows = Array.isArray(kyc) ? kyc : [];

      const normalizeDate = (value) =>
        value ? String(value).split("T")[0] : "";

      const normalizedLoan = Object.fromEntries(
        Object.entries(resumedLoan).map(([key, value]) => [
          key,
          key.includes("date") ? normalizeDate(value) : value,
        ]),
      );
      ...
      const gstRow = kycRows.find((row) => row?.gst_status) ?? null;

      setGstVerified(gstRow?.gst_status === "VERIFIED");

      const borrowers = [];
      const resumedCoApplicants = [];
      const resumedGuarantors = [];

      applicantRows.forEach((row) => {
        const normalizedApplicant = {
          ...emptyApplicant(row.role || "BORROWER", row.party_no || 1),
          ...row,
          dob: normalizeDate(row.dob),
          _uid:
            row._uid ||
            `${row.role}_${row.party_no}_${
              row.id || Math.random().toString(36).slice(2)
            }`,
        };

        if (row.role === "BORROWER") {
          borrowers.push(normalizedApplicant);
        } else if (row.role === "CO_APPLICANT") {
          resumedCoApplicants.push(normalizedApplicant);
        } else if (row.role === "GUARANTOR") {
          resumedGuarantors.push(normalizedApplicant);
        }
      });

      if (borrowers.length > 0) {
        setApplicant(borrowers[0]);

        setOtpVerified((previous) => ({
          ...previous,
          applicant: Boolean(borrowers[0].mobile_verified),
        }));
      }

      if (resumedCoApplicants.length > 0) {
        setCoApplicants(resumedCoApplicants);

        const verifiedState = {};

        resumedCoApplicants.forEach((row, index) => {
          verifiedState[`co_${index}`] = Boolean(row.mobile_verified);
        });
      ...
      if (resumedGuarantors.length > 0) {
        setGuarantors(resumedGuarantors);

        const verifiedState = {};

        resumedGuarantors.forEach((row, index) => {
          verifiedState[`guarantor_${index}`] = Boolean(row.mobile_verified);
        });
      ...
      if (normalizedLoan.business_name && normalizedLoan.business_mobile) {
        completed.add(0);
      }
      ...
      if (normalizedLoan.loan_amount && normalizedLoan.loan_tenure) {
        completed.add(2);
      }

      if (
        normalizedLoan.bank_name &&
        normalizedLoan.account_number &&
        normalizedLoan.ifsc
      ) {
        completed.add(3);
      }
      ...
      setMessage(
        `❌ ${
          ...
          err.message ||
          "Could not load existing loan details."
        }`,
      );
    } finally {
      setLoading(false);
  ...
  const isValidPan = (p) =>
    /^[A-Z]{5}[0-9]{4}[A-Z]$/.test(String(p || "").trim());
  const isValidEmail = (e) =>
    !e || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(e || "").trim());
  const isValidGstin = (g) =>
    /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z][a-zA-Z0-9][0-9A-Z]$/.test(
      String(g || "")
        .trim()
        .toUpperCase(),
    );
  ...
  const handlePincodeLookup = async (pincode, target) => {
    ...
    try {
      ...
      if (data.Status !== "Success" || !data.PostOffice?.length) {
        setMessage(
          "⚠️ Pincode not found — please fill city/district/state manually.",
        );
        return;
      }
      ...
      setMessage(
        "⚠️ Could not fetch pincode details — please fill city/district/state manually.",
      );
    }
  };

  const handleIfscLookup = async (ifscValue) => {
    const ifsc = String(ifscValue || "")
      .trim()
      .toUpperCase();
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (String(applicant.current_pincode || "").length === 6)
      handlePincodeLookup(applicant.current_pincode, "applicant_current");
  }, [applicant.current_pincode]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (String(applicant.permanent_pincode || "").length === 6)
      handlePincodeLookup(applicant.permanent_pincode, "applicant_permanent");
  }, [applicant.permanent_pincode]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (String(loan.business_pincode || "").length === 6)
      handlePincodeLookup(loan.business_pincode, "business");
  }, [loan.business_pincode]);
  useEffect(() => {
    return () => {
      Object.values(otpTimersRef.current).forEach(clearInterval);
    };
  }, []);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    coApplicants.forEach((co, i) => {
      if (String(co.current_pincode || "").length === 6)
        handlePincodeLookup(co.current_pincode, `co_${i}_current`);
    });
  }, [coApplicants.map((c) => c.current_pincode).join(",")]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    coApplicants.forEach((co, i) => {
      if (String(co.permanent_pincode || "").length === 6)
        handlePincodeLookup(co.permanent_pincode, `co_${i}_permanent`);
    });
  }, [coApplicants.map((c) => c.permanent_pincode).join(",")]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    guarantors.forEach((g, i) => {
      if (String(g.current_pincode || "").length === 6)
        handlePincodeLookup(g.current_pincode, `guarantor_${i}_current`);
    });
  }, [guarantors.map((g) => g.current_pincode).join(",")]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    guarantors.forEach((g, i) => {
      if (String(g.permanent_pincode || "").length === 6)
        handlePincodeLookup(g.permanent_pincode, `guarantor_${i}_permanent`);
    });
  }, [guarantors.map((g) => g.permanent_pincode).join(",")]);
    ...
    switch (name) {
      ...
      case "mobile":
        ...
        return isValidMobile(v)
          ? ""
          : "Enter valid 10-digit mobile (starts 6–9)";
      ...
      case "business_pan":
        ...
        return isValidPan(v) ? "" : "Invalid PAN (e.g. ABCDE1234F)";
      ...
      case "permanent_pincode":
        return v && !/^\d{6}$/.test(v) ? "Enter valid 6-digit pincode" : "";

      case "business_pincode":
        if (!v) {
          return "Business pincode is required";
        }

        return /^\d{6}$/.test(v) ? "" : "Enter valid 6-digit pincode";

      case "current_address":
        return v ? "" : "Current address is required";

      case "login_date":
        return v ? "" : "Login date is required";
      ...
      case "loan_tenure":
        return !v || Number(v) <= 0 ? "Tenure must be greater than zero" : "";
      ...
      case "other_charges":
        ...
        return Number(v) < 0 ? "Value cannot be negative" : "";

      case "business_name":
        return v ? "" : "Business name is required";

      case "trade_name":
        return v ? "" : "Trade name is required";

      case "business_start_date":
        return v ? "" : "Business start date is required";

      case "business_vintage_months":
        return v ? "" : "Business vintage is required";

      case "business_address":
        return v ? "" : "Business address is required";

      case "business_city":
        return v ? "" : "Business city is required";

      case "business_district":
        return v ? "" : "Business district is required";

      case "business_state":
        return v ? "" : "Business state is required";

      case "constitution_type":
        return v ? "" : "Constitution type is required";

      case "premises_ownership":
        return v ? "" : "Premises ownership is required";

      case "business_mobile":
        if (!v) {
          return "Business mobile is required";
        }
      ...
      case "name_in_bank":
        return v ? "" : "Name in bank is required";

      case "account_number":
        return v ? "" : "Account number is required";

      case "account_type":
        return v ? "" : "Account type is required";
  ...
  const validateApplicant = () => {
    const e = {};
    if (!applicant.first_name && !applicant.full_name)
      e["applicant_first_name"] = "First Name is required";
    if (!isValidMobile(applicant.mobile))
      e["applicant_mobile"] = "Valid 10-digit mobile required";
    if (applicant.pan && !isValidPan(applicant.pan))
      e["applicant_pan"] = "Invalid PAN format";
    if (applicant.email && !isValidEmail(applicant.email))
      e["applicant_email"] = "Invalid email format";
    if (!applicant.dob) e["applicant_dob"] = "Date of birth required";
    if (!applicant.current_address)
      e["applicant_current_address"] = "Current address required";
    if (
      applicant.current_pincode &&
      String(applicant.current_pincode).length !== 6
    )
      e["applicant_current_pincode"] = "6-digit pincode required";
    return e;
  };
  ...
  const validateBusiness = () => {
    ...
    if (!loan.constitution_type)
      e["constitution_type"] = "Constitution type is required";
    if (!loan.business_mobile || !isValidMobile(loan.business_mobile))
      e["business_mobile"] = "Valid 10-digit mobile required";
    if (loan.business_email && !isValidEmail(loan.business_email))
      e["business_email"] = "Invalid email format";
    if (loan.gstin && !isValidGstin(loan.gstin))
      e["gstin"] = "Invalid GSTIN format";
    if (loan.business_pan && !isValidPan(loan.business_pan))
      e["business_pan"] = "Invalid PAN format";
    if (loan.business_pincode && String(loan.business_pincode).length !== 6)
      e["business_pincode"] = "6-digit pincode required";
    return e;
  };

  const validateLoan = () => {
    const e = {};
    if (!loan.login_date) e["login_date"] = "Login date is required";
    if (!loan.loan_amount || Number(loan.loan_amount) <= 0)
      e["loan_amount"] = "Loan amount must be > 0";
    if (!loan.interest_rate || Number(loan.interest_rate) <= 0)
      e["interest_rate"] = "Interest rate is required and must be > 0";
    if (!loan.loan_tenure || Number(loan.loan_tenure) <= 0)
      e["loan_tenure"] = "Tenure is required and must be > 0 months";
    if (!loan.processing_fee || Number(loan.processing_fee) < 0)
      e["processing_fee"] = "Processing fee is required";
    if (!loan.insurance_amount || Number(loan.insurance_amount) < 0)
      e["insurance_amount"] = "Insurance amount is required";
    if (!loan.other_charges || Number(loan.other_charges) < 0)
      e["other_charges"] = "Other charges is required";
    return e;
  };
  ...
  const validateBank = () => {
    ...
    if (!loan.name_in_bank) e["name_in_bank"] = "Name in bank is required";
    if (!loan.account_number)
      e["account_number"] = "Account number is required";
    if (!loan.ifsc) e["ifsc"] = "IFSC code is required";
    if (!loan.account_type) e["account_type"] = "Account type is required";
    return e;
  };
  ...
  const validatePartyList = (parties, prefix) => {
    ...
    parties.forEach((a, i) => {
      ...
      if (a.email && !isValidEmail(a.email))
        e[`${p}_email`] = "Invalid email format";
    });
    return e;
  };
  ...
  const getSectionErrors = (sectionIdx) => {
    switch (sectionIdx) {
      ...
      case 3:
        return validateBank();
      case 4: {
        const filled = coApplicants.filter(
          (a) => a.first_name || a.full_name || a.mobile,
        );
        return filled.length > 0 ? validatePartyList(filled, "co") : {};
      }
      case 5: {
        const filled = guarantors.filter(
          (a) => a.first_name || a.full_name || a.mobile,
        );
  ...
  const handleLoanChange = (e) => {
    ...
    if (["business_pincode"].includes(name))
      value = value.replace(/\D/g, "").slice(0, 6);
    if (name === "gstin") {
      setGstVerified(false);
      setGstRawResponse(null);
    }
    ...
    setLoan((prev) => {
      ...
      if (name === "loan_amount" || name === "processing_fee") {
        ...
        const fee = Number(
          name === "processing_fee" ? value : up.processing_fee,
        );
        if (amt > 0) {
          up.processing_fee_percentage =
            fee >= 0
              ? ((fee / amt) * 100).toFixed(2)
              : up.processing_fee_percentage;
  ...
  const handleApplicantChange =
    ...
    (e) => {
      ...
      if (
        ["pan", "voter_id", "driving_license_no", "passport_no"].includes(name)
      )
        finalValue = String(value).toUpperCase();
      if (name === "email") finalValue = value.toLowerCase().replace(/\s/g, "");
      if (["current_pincode", "permanent_pincode"].includes(name))
  ...
  const sendOtp = async (mobile, targetKey) => {
    try {
      setOtpLoading(true);
      const res = await api.post("/fundify/send-otp", {
        mobile,
        applicantType: toApplicantType(targetKey),
      });
      if (res.data.success) {
        setResendTimers((p) => ({ ...p, [targetKey]: 60 }));

        if (otpTimersRef.current[targetKey])
          clearInterval(otpTimersRef.current[targetKey]);
        ...
        const timer = setInterval(() => {
          setResendTimers((p) => {
            ...
            if (cur <= 1) {
              clearInterval(timer);
              delete otpTimersRef.current[targetKey];
              return { ...p, [targetKey]: 0 };
            }
            return { ...p, [targetKey]: cur - 1 };
          });
        }, 1000);

        otpTimersRef.current[targetKey] = timer;
      }
    } catch (err) {
      alert(
  ...
  const verifyOtpHandler = async () => {
    ...
    else if (verificationTarget.startsWith("co_"))
      mobile =
        coApplicants[parseInt(verificationTarget.split("_")[1], 10)]?.mobile ||
        "";
    else if (verificationTarget.startsWith("guarantor_"))
      mobile =
        guarantors[parseInt(verificationTarget.split("_")[1], 10)]?.mobile ||
        "";
  ...
  const saveBusinessSection = async () => {
    if (businessSaved && lan) return true;
    try {
      setLoading(true);
      ...
      setMessage(
        `❌ ${res.data.message || "Failed to save business details."}`,
      );
      return false;
    } catch (err) {
      setLan("");
  ...
  const updateApplicantSection = async () => {
    if (!otpVerified.applicant) {
      setMessage(
        "❌ Please verify applicant's mobile via OTP before proceeding.",
      );
      return false;
    }
    if (!lan) {
      setMessage("❌ LAN missing. Please complete Business Details first.");
      return false;
    }
    try {
      setLoading(true);
      ...
      setMessage(
        `❌ ${err.response?.data?.message || "Failed to save applicant"}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const updateSection = async (section) => {
    if (!lan) {
      setMessage("❌ LAN missing.");
      return false;
    }
    try {
      setLoading(true);
      const res = await api.post("/fundify/update-section", {
        lan,
        section,
        data: loan,
      });
      if (res.data?.success) return true;
      setMessage(`❌ ${res.data?.message || "Failed to save section"}`);
      return false;
    } catch (err) {
      setMessage(
  ...
  const saveParties = async (parties, role) => {
    ...
    try {
      ...
      for (let i = 0; i < filled.length; i++) {
        const targetKey =
          role === "CO_APPLICANT" ? `co_${i}` : `guarantor_${i}`;
        await api.post("/fundify/save-party", {
          ...
          applicant: {
            ...
            role,
            party_no: i + 1,
            mobile_verified: otpVerified[targetKey] ? 1 : 0,
          },
        });
      }
      return true;
  ...
  const finalSubmit = async () => {
    if (!lan) {
      setMessage("❌ LAN missing. Cannot submit.");
      return false;
    }
    try {
      setLoading(true);
      ...
      setMessage(
        `❌ ${err.response?.data?.message || "Failed to submit loan"}`,
      );
      return false;
    } finally {
      setLoading(false);
  ...
  const handleNext = async () => {
    ...
    if (Object.keys(sectionErrors).length > 0) {
      ...
      setTouched((prev) => ({
        ...prev,
        ...Object.keys(sectionErrors).reduce(
          (acc, k) => ({ ...acc, [k]: true }),
          {},
        ),
      }));
      setMessage(
        `❌ Please fix errors in "${SECTIONS[activeSection]}" before proceeding.`,
      );
      return;
    }
    setErrors({});
    ...
    let saved = false;
    if (activeSection === 0)
      saved = await saveBusinessSection(); // creates LAN
    else if (activeSection === 1)
      saved = await updateApplicantSection(); // uses existing LAN
    else if (activeSection === 2) saved = await updateSection("loan");
    else if (activeSection === 3) saved = await updateSection("bank");
    else if (activeSection === 4)
      saved = await saveParties(coApplicants, "CO_APPLICANT");
  ...
  const parseGstAddress = (address = "") => {
    ...
    if (parts.length >= 4) {
      ...
      result.city = parts[parts.length - 4] || parts[parts.length - 3] || "";
    }
    return result;
  };

  const handleVerifyGst = async () => {
    const gstin = String(loan.gstin || "")
      .trim()
      .toUpperCase();
    if (!gstin) {
      setErrors((p) => ({ ...p, gstin: "GSTIN is required" }));
      return;
    }
    if (!isValidGstin(gstin)) {
      setErrors((p) => ({ ...p, gstin: "Invalid GSTIN format" }));
      return;
    }
    try {
      setGstLoading(true);
    ...
    return (
      <div className="form-group">
        ...
        <div className="input-wrapper">
          ...
          {isTouched && !readOnly && (
            <span
              className={`val-icon ${status === "valid" ? "val-valid" : status === "invalid" ? "val-invalid" : ""}`}
            >
        ...
        {status === "valid" && isTouched && !readOnly && (
    ...
    return (
      <div className="form-group">
        ...
        <select
          ...
          onChange={(e) => {
            onChange(e);
            markField(key, e.target.value, name);
          }}
        >
  ...
  const isAadhaarButtonDisabled = (stateKey) =>
    ["INITIATING", "INITIATED", "VERIFIED", "COMPLETED"].includes(
      aadhaarStatus[stateKey],
    );

  const isSectionLocked = (sectionIdx) => completedSections.has(sectionIdx);
  ...
  const triggerAadhaar = async (applicantType, stateKey) => {
    if (!lan) {
      setMessage("❌ Please save applicant first to generate LAN.");
      return;
    }
    try {
      setAadhaarStatus((prev) => ({ ...prev, [stateKey]: "INITIATING" }));
      const res = await api.post("/fundify/init-aadhaar", {
        lan,
        applicantType,
      });
      if (res.data.success) {
      ...
      setAadhaarStatus((prev) => ({ ...prev, [stateKey]: "FAILED" }));
      setMessage(
        `❌ ${err.response?.data?.message || `Aadhaar initiation failed for ${applicantType}`}`,
      );
    }
  };
  ...
  const fetchAndPrefillAadhaarAddress = async (applicantType, setter) => {
    ...
    try {
      setLoading(true);
      const res = await api.get(
        `/fundify/aadhaar-address/${lan}/${applicantType}`,
      );
      if (!res.data.success) {
      ...
      setMessage(
        `❌ ${err.response?.data?.message || "Aadhaar address not available yet."}`,
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="party-card">
      ...
      <div className="form-grid">
        ...
        {aadhaarReady && applicantType && stateKey && (
          <div
            className="aadhaar-btn-row full-row"
            style={{
              display: "flex",
              gap: "10px",
              flexWrap: "wrap",
          ...
          >
            <button
              type="button"
              className={
                isAadhaarButtonDisabled(stateKey)
                  ? "verified-btn"
            ...
            <button
              ...
              className="otp-btn"
              onClick={() =>
                fetchAndPrefillAadhaarAddress(applicantType, aadhaarSetter)
              }
        ...
        <div className="form-group">
          {renderInput(
            ...
            "text",
            false,
            `${prefix}_current_pincode`,
            true,
          )}
          <small
            style={{ color: "#94a3b8", fontSize: "11px", marginTop: "-4px" }}
          >
            City &amp; district auto-fill on 6 digits
          </small>
        </div>
        {renderInput(
          "Current Landmark",
        ...
        {renderSelect(
          ...
          onChange,
          ["Owned", "Rented", "Leased", "Family Owned", "Other"],
          `${prefix}_residence_ownership`,
          true,
        )}
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Thank you! Your loan application has been created and forwarded to
            the
            <strong style={{ color: "#cbd5e1" }}> Credit Team</strong> for
            review and verification. You will be notified once the process is
            complete.
          </p>
          ...
          >
            ...
            <span
              style={{ fontSize: "13px", color: "#fde68a", fontWeight: 500 }}
            >
    ...
    <div className="manual-entry-container">
      ...
      <div className="section-tabs">
        {SECTIONS.map((sec, idx) => (
          ...
          >
            {completedSections.has(idx) && activeSection !== idx ? "✓ " : ""}
            {sec}
            {(idx === 4 || idx === 5) && (
            ...
            <div className="form-grid">
              <div className="gst-verify-wrapper">
                ...
                <button
                  ...
                  disabled={gstLoading || gstVerified || isSectionLocked(0)}
                >
                  {gstLoading
                    ? "Verifying..."
                    : gstVerified
              ...
              {renderInput(
                ...
                "shop_establishment_no",
                loan.shop_establishment_no,
                handleLoanChange,
              )}
              {renderInput(
                "Business Reg No",
                "business_registration_no",
                loan.business_registration_no,
                handleLoanChange,
              )}
              {renderSelect(
                "Constitution Type",
                "constitution_type",
                loan.constitution_type,
                handleLoanChange,
          ...
          <div className="party-card">
            ...
            <div className="form-grid">
              ...
              {renderInput(
                "Processing Fee %",
                "processing_fee_percentage",
                loan.processing_fee_percentage,
                handleLoanChange,
                "number",
                true,
              )}
              ...
              <div className="form-group">
                ...
                <div className="input-wrapper">
                  <input
                    ...
                    readOnly={isSectionLocked(3)}
                    onBlur={() => {
                      if (isSectionLocked(3)) return;
                      markField("ifsc", loan.ifsc, "ifsc");
                      handleIfscLookup(loan.ifsc);
                    }}
                  ...
                  {touched["ifsc"] && (
                    <span
                      className={`val-icon ${fieldStatus["ifsc"] === "valid" ? "val-valid" : fieldStatus["ifsc"] === "invalid" ? "val-invalid" : ""}`}
                    >
                      {fieldStatus["ifsc"] === "valid"
                        ? "✓"
                        : fieldStatus["ifsc"] === "invalid"
                          ? "✕"
                          : ""}
                    </span>
                  )}
                ...
                <small style={{ color: "#94a3b8", fontSize: "11px" }}>
                  Bank name auto-fills when you leave this field
                </small>
              </div>
        ...
        {activeSection === 4 && (
          <>
            <div className="optional-notice">
              <span>ℹ️</span>
              <span>
                Co-Applicants are <strong>optional</strong>. You can skip this
                section and submit the case.
              </span>
            </div>
            ...
            <button
              ...
              onClick={() =>
                setCoApplicants((p) => [
                  ...p,
                  emptyApplicant("CO_APPLICANT", p.length + 1),
                ])
              }
            >
        ...
        {activeSection === 5 && (
          <>
            <div className="optional-notice">
              <span>ℹ️</span>
              <span>
                Guarantors are <strong>optional</strong>. You can skip and
                submit the case directly.
              </span>
            </div>
              ...
              <div key={idx}>
                ...
                {guarantors.length > 1 && (
                  <button
                    ...
                    className="remove-btn"
                    onClick={() =>
                      setGuarantors((p) => p.filter((_, i) => i !== idx))
                    }
                  >
            ...
            <button
              ...
              onClick={() =>
                setGuarantors((p) => [
                  ...p,
                  emptyApplicant("GUARANTOR", p.length + 1),
                ])
              }
            >
      ...
      {message && (
        <div
          className={`message ${message.includes("❌") ? "message-error" : "message-success"}`}
        >
          {message}
        </div>
      )}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (verificationTarget === "applicant") m = applicant.mobile;
                    ...
                    m =
                      coApplicants[
                        parseInt(verificationTarget.split("_")[1], 10)
                      ]?.mobile || "";
                  else if (verificationTarget.startsWith("guarantor_"))
                    m =
                      guarantors[parseInt(verificationTarget.split("_")[1], 10)]
                        ?.mobile || "";
  ...
  .lan-badge {
    ...
    color: white; border-radius: 30px; padding: 7px 16px;
  }
  .lan-label { font-size: 11px; font-weight: 600; opacity: 0.7; text-transform: uppercase; }
  .lan-value { font-size: 13px; font-weight: 700; font-family: monospace; }
  ...
  .optional-badge {
    ...
    background: rgba(255,255,255,0.3); padding: 2px 5px; border-radius: 4px;
  ...
  .party-card h3 {
    margin: 0 0 18px; font-size: 16px; font-weight: 700; color: #0f172a;
  ...
  .val-icon { position: absolute; right: 10px; font-size: 13px; font-weight: 700; pointer-events: none; }
  ...
  /* ── MOBILE + OTP ROW ──
     Input grows, button stays content-width — never stretches */
  .mobile-otp-wrapper {
    display: flex;
    gap: 10px;
  ...
  .mobile-otp-wrapper .otp-btn,
  .mobile-otp-wrapper .verified-btn {
    flex-shrink: 0;
    width: auto;
    padding: 0 20px;
    height: 42px;
  ...
  .add-btn {
    background: #f0fdf4; color: #15803d; border: 1.5px dashed #86efac;
    margin: 10px 0; display: block; width: 100%;
  }
  ...
  /* ── MESSAGES ── */
  .message { margin-top: 16px; padding: 12px 16px; border-radius: 12px; font-size: 14px; font-weight: 500; }
  ...
  .modal-card h3 { margin: 0 0 16px; font-size: 18px; font-weight: 700; color: #0f172a; }
  .consent-scroll {
    height: 160px; overflow-y: auto; border: 1px solid #e2e8f0;
  ...
  .checkbox-container {
    ...
    margin-bottom: 16px; font-size: 14px; font-weight: 500; color: #1e293b; cursor: pointer;
  }
  .checkbox-container input { width: 16px; height: 16px; }
  .resend-timer { margin-top: 10px; font-size: 13px; color: #64748b; text-align: center; }
  ...
  @media (max-width: 768px) {
    ...
    /* OTP row stacks vertically on mobile */
    .mobile-otp-wrapper { flex-direction: column; align-items: stretch; }
    .mobile-otp-wrapper .otp-btn,
    .mobile-otp-wrapper .verified-btn { width: 100%; height: 42px; padding: 0; }

    /* GST verify stacks */
    .gst-verify-wrapper { flex-direction: column; align-items: stretch; }
    .gst-verify-wrapper .verify-btn,
    .gst-verify-wrapper .verified-btn { width: 100%; height: 42px; }

    /* Step buttons full-width, stacked */
    .step-buttons { flex-direction: column-reverse; }
    .back-btn, .skip-btn, .next-btn, .submit-btn { width: 100%; text-align: center; }

    /* Modal */
    .modal-card { border-radius: 14px; padding: 18px; }

    /* Cards */
    .party-card { padding: 14px; }

    /* Aadhaar button row */
    .aadhaar-btn-row { flex-direction: column !important; }
    .aadhaar-btn-row button { min-width: unset !important; width: 100% !important; }
  }
`}</style>
    </div>
  );
