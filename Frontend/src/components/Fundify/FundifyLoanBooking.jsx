...
//     if (!a.full_name && !a.first_name) e[`${prefix}_name`] = "Name required";
//     if (!a.mobile || !isValidMobile(a.mobile))
//       e[`${prefix}_mobile`] = "Valid mobile required";
//     if (!a.pan || !isValidPan(a.pan)) e[`${prefix}_pan`] = "Valid PAN required";
//     if (!isValidEmail(a.email)) e[`${prefix}_email`] = "Invalid email";
//     if (!a.dob) e[`${prefix}_dob`] = "DOB required";
//     if (!a.current_address) e[`${prefix}_address`] = "Address required";
//     if (!a.current_pincode) e[`${prefix}_pincode`] = "Pincode required";
...
//       e.loan_amount = "Valid loan amount required";
//     }

//     if (loan.interest_rate && Number(loan.interest_rate) <= 0) {
//       e.interest_rate = "Valid interest rate required";
//     }

//     if (loan.loan_tenure && Number(loan.loan_tenure) <= 0) {
//       e.loan_tenure = "Valid tenure required";
//     }
...
//       e.business_mobile = "Valid business mobile required";
//     }

//     if (loan.business_email && !isValidEmail(loan.business_email)) {
//       e.business_email = "Invalid business email";
//     }

//     if (loan.business_pan && !isValidPan(loan.business_pan)) {
//       e.business_pan = "Invalid business PAN";
//     }

//     if (loan.business_pincode && String(loan.business_pincode).length !== 6) {
//       e.business_pincode = "Valid 6 digit pincode required";
//     }
...
//     if (!loan.ifsc) e.ifsc = "IFSC required";
...
//       e = {
//         ...e,
//         ...validateApplicant(a, `guarantor_${i}`),
//       };
//     });
...
//   return Object.keys(getSectionErrors(activeSection)).length === 0;
// };
...
//     e = { ...e, ...validateApplicant(applicant, "applicant") };
...
//       const res = await api.post("/fundify/manual-entry", buildPayload());

//       setMessage(
//         `✅ Fundify loan created successfully | LAN: ${res.data.lan} | Partner Loan ID: ${res.data.partner_loan_id}`
//       );
//     } catch (err) {
//       setMessage(
...
//   const renderInput = (label, name, value, onChange, type = "text", readOnly = false) => (
...
//         {renderInput("First Name", "first_name", data.first_name, onChange)}
//         {renderInput("Middle Name", "middle_name", data.middle_name, onChange)}
//         {renderInput("Last Name", "last_name", data.last_name, onChange)}
//         {renderInput("Full Name", "full_name", data.full_name, onChange)}
//         {renderInput("Father Name", "father_name", data.father_name, onChange)}
//         {renderInput("Mother Name", "mother_name", data.mother_name, onChange)}
//         {renderInput("Spouse Name", "spouse_name", data.spouse_name, onChange)}
//         {renderInput("DOB", "dob", data.dob, onChange, "date")}

//         {renderSelect("Gender", "gender", data.gender, onChange, [
//           "Male",
//           "Female",
//           "Other",
//         ])}

//         {renderSelect("Marital Status", "marital_status", data.marital_status, onChange, [
//           "Single",
//           "Married",
//           "Divorced",
//           "Widowed",
...
//         {renderInput("Mobile", "mobile", data.mobile, onChange)}
//         {renderInput("Alternate Mobile", "alternate_mobile", data.alternate_mobile, onChange)}
//         {renderInput("Email", "email", data.email, onChange, "email")}
//         {renderInput("PAN", "pan", data.pan, onChange)}
//         {renderInput("Aadhaar Last 4", "aadhaar_last4", data.aadhaar_last4, onChange)}
//         {renderInput("Voter ID", "voter_id", data.voter_id, onChange)}
//         {renderInput("Driving License No", "driving_license_no", data.driving_license_no, onChange)}
//         {renderInput("Passport No", "passport_no", data.passport_no, onChange)}
//         {renderInput("CKYC No", "ckyc_no", data.ckyc_no, onChange)}

//         {renderInput("Current Address", "current_address", data.current_address, onChange)}
//         {renderInput("Current City", "current_city", data.current_city, onChange)}
//         {renderInput("Current District", "current_district", data.current_district, onChange)}
//         {renderInput("Current State", "current_state", data.current_state, onChange)}
//         {renderInput("Current Pincode", "current_pincode", data.current_pincode, onChange)}
//         {renderInput("Current Landmark", "current_landmark", data.current_landmark, onChange)}

//         {renderSelect("Residence Ownership", "residence_ownership", data.residence_ownership, onChange, [
//           "Owned",
//           "Rented",
//           "Leased",
//           "Family Owned",
...
//         {renderInput("Permanent Address", "permanent_address", data.permanent_address, onChange)}
//         {renderInput("Permanent City", "permanent_city", data.permanent_city, onChange)}
//         {renderInput("Permanent District", "permanent_district", data.permanent_district, onChange)}
//         {renderInput("Permanent State", "permanent_state", data.permanent_state, onChange)}
//         {renderInput("Permanent Pincode", "permanent_pincode", data.permanent_pincode, onChange)}

//         {renderInput("Occupation", "occupation", data.occupation, onChange)}
//         {renderInput("Employer Name", "employer_name", data.employer_name, onChange)}
//         {renderInput("Monthly Income", "monthly_income", data.monthly_income, onChange, "number")}
//       </div>

//       {Object.entries(errors)
//         .filter(([key]) => key.startsWith(errorPrefix))
//         .map(([key, value]) => (
//           <p className="error-text" key={key}>{value}</p>
//         ))}
//     </div>
//   );
...
//       )
//       .map(([key, value]) => (
//         <p className="error-text" key={key}>
//           {value}
//         </p>
//       ))}
//   </>
...
//             {renderSelect("Premises Ownership", "premises_ownership", loan.premises_ownership, handleLoanChange, [
//               "Owned",
//               "Rented",
//               "Leased",
//               "Family Owned",
...
//             {renderSelect("Account Type", "account_type", loan.account_type, handleLoanChange, [
//               "SAVINGS",
//               "CURRENT",
//               "OD",
//               "CC",
...
//           {Object.entries(errors)
//       .filter(([key]) =>
//         ["bank_name", "account_number", "ifsc"].includes(key)
//       )
//       .map(([key, value]) => (
//         <p className="error-text" key={key}>
//           {value}
...
//                 {renderApplicantForm(
//                   co,
//                   handleApplicantChange(setCoApplicants, index),
...
//                   ...prev,
//                   emptyApplicant("CO_APPLICANT", prev.length + 1),
//                 ])
//               }
//             >
//               + Add Co-Applicant
...
//                 {renderApplicantForm(
//                   g,
//                   handleApplicantChange(setGuarantors, index),
//                   `Guarantor ${index + 1}`,
//                   `guarantor_${index}`
//                 )}
...
//                 setGuarantors((prev) => [
//                   ...prev,
//                   emptyApplicant("GUARANTOR", prev.length + 1),
//                 ])
//               }
//             >
//               + Add Guarantor
...
//         setMessage("❌ Please complete the current section first.");
//         return;
//       }
