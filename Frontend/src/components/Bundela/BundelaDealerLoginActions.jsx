...
const BundelaDealerLoginActions = () => {
  ...
  useEffect(() => {
    ...
    api
      .get("/bundela/dealers-login-cases")
      .then((res) => {
  ...
  const handleStatusChange = async (lan, status) => {
    try {
      await api.patch(`/bundela/dealer/status/${lan}`, {
  ...
  const columns = [
    {
      ...
      render: (r) => (
        <span
          ...
          onClick={() => navigate(`/bundela/dealer-details/${r.lan}`)}
        >
          {r.business_name}
        </span>
      ),
  ...
  return (
    <DataTable
      title="Bundela Dealer Credit Approval"
      rows={rows}
      columns={columns}
      globalSearchKeys={[
        "business_name",
      ...
      exportFileName="bundela_dealers"
    />
  );
};

export default BundelaDealerLoginActions;