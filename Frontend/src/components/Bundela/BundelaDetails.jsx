...
const BundelaDetails = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        setLoading(true);
        setErr("");
        const res = await api.get(`/bundela/customer-details/${lan}`);
        setDetails(res.data);
      } catch (e) {
        console.error("Failed to fetch Bundela details:", e);
  ...
  const sections = [
    ...
    },
    {
      ...
      content: hasValue(loan.guarantor?.name) ? (
        <Grid>
          ...
          <Field label="Relationship With Borrower" value={loan.guarantor?.relationship_with_borrower} />
          <Field label="Address Line 1" value={loan.guarantor?.address?.address_line_1} />
          <Field label="Address Line 2" value={loan.guarantor?.address?.address_line_2} />
        ...
        <p style={{ margin: 0, color: "#64748b", fontWeight: 700 }}>No guarantor details available.</p>
      ),
    },
    {
      ...
      content: hasValue(loan.co_applicant?.name) ? (
        <Grid>
          ...
          <Field label="Co-Applicant DOB" value={formatDate(loan.co_applicant?.dob)} />
          <Field label="Address Line 1" value={loan.co_applicant?.address?.address_line_1} />
          <Field label="Address Line 2" value={loan.co_applicant?.address?.address_line_2} />
        ...
        <p style={{ margin: 0, color: "#64748b", fontWeight: 700 }}>No co-applicant details available.</p>
      ),
    },
    {
      ...
      content: (
        <Grid>
          <Field label="Requested Loan Amount" value={`₹${loan.loan_details?.requested_loan_amount || "-"}`} highlight />
          <Field label="Loan Amount" value={`₹${loan.loan_details?.loan_amount || "-"}`} highlight />
          <Field label="Tenure" value={`${loan.loan_details?.loan_tenure || 0} Months`} />
          <Field label="Interest Rate" value={`${loan.loan_details?.interest_rate || 0}%`} />
          <Field label="Processing Fee" value={`₹${loan.loan_details?.processing_fee || "-"}`} />
          <Field label="Processing Fee %" value={loan.loan_details?.processing_fee_percentage} />
          <Field label="Disbursal Amount" value={loan.loan_details?.disbursal_amount} />
    ...
    },
    {
      ...
      content: (
        <Grid>
          <Field label="Bank Name" value={loan.bank_details?.customer_bank_name} />
          <Field label="Account Holder" value={loan.bank_details?.customer_name_as_per_bank} />
          <Field label="Account Number" value={loan.bank_details?.customer_account_number} />
    ...
    },
    {
      ...
      content: (
        <Grid>
          <Field label="Address Line 1" value={loan.permanent_address?.address_line_1} />
          <Field label="Address Line 2" value={loan.permanent_address?.address_line_2} />
    ...
    },
    {
      ...
      content: (
        <Grid>
          ...
          <Field label="Dealer Contact" value={loan.dealer_details?.dealer_contact} />
          <Field label="Dealer Email" value={loan.dealer_details?.dealer_email} />
          <Field label="GST Number" value={loan.dealer_details?.gst_no} />
          <Field label="Dealer City" value={loan.dealer_details?.dealer_city} />
          <Field label="Dealer State" value={loan.dealer_details?.dealer_state} />
        </Grid>
      ),
    },
    {
      ...
      content: (
        <div style={{ display: "grid", gap: "24px" }}>
          <ApplicantVerificationBlock title="Borrower" data={loan.verification_status?.borrower} />
          {hasValue(loan.guarantor?.name) && (
            <ApplicantVerificationBlock title="Guarantor" data={loan.verification_status?.guarantor} />
          )}
          {hasValue(loan.co_applicant?.name) && (
            <ApplicantVerificationBlock title="Co-Applicant" data={loan.verification_status?.co_applicant} />
          )}
        </div>
      ),
    },
      ...
      <div style={{ maxWidth: "1300px", margin: "0 auto" }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '40px' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <span style={{ fontSize: '14px', color: '#64748b', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Bundela Profile</span>
...
const ApplicantVerificationBlock = ({ title, data }) => {
  ...
  return (
    <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "18px", padding: "24px" }}>
      <h4 style={{ margin: "0 0 20px 0", fontSize: "18px", fontWeight: 900, color: "#0f172a" }}>
        {title}
      </h4>
      <Grid>
        <VerificationField label="PAN Status" value={status.pan_status} />
        <VerificationField label="Aadhaar Status" value={status.aadhaar_status} />
...
const VerificationField = ({ label, value }) => {
  ...
  const statusColors = {
    ...
    INITIATED: { bg: "#dbeafe", text: "#1e40af", border: "#bfdbfe" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
      <label style={{ fontSize: "12px", color: "#94a3b8", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em" }}>
        {label}
      </label>
      <span style={{ padding: "9px 18px", borderRadius: "999px", fontSize: "14px", fontWeight: 900, background: c.bg, color: c.text, border: `1px solid ${c.border}`, width: "fit-content" }}>
        {status}
      </span>
    </div>
  );
