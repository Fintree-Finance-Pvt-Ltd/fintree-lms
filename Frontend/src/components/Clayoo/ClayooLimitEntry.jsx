...
const ClayooLimitEntry = ({
  ...
  const handleLimitSubmit = async (lan) => {
    ...
    try {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.lan === lan
            ? { ...r, final_limit: limit, status: "LIMIT REQUESTED" }
            : r
        )
      );
    } catch {
  ...
  const handleAgreementEsign = async (row) => {
    ...
    if (!window.confirm("Send agreement eSign?")) return;

    setActionLan(row.lan);

    try {
      await api.post(`/esign/${row.lan}/esign/agreement`);
      ...
      setToast({ type: "success", msg: "Agreement initiated" });
    } catch {
      setToast({ type: "error", msg: "Failed to initiate agreement" });
    } finally {
      setActionLan(null);
    }
  };
  ...
  const handleBankSubmit = async (e) => {
    ...
    try {
      ...
      const verify = await api.post("/enach/verify-bank", {
        lan,
        ...bankForm,
        amount: 1,
      });
      ...
      const mandate = await api.post("/enach/create-mandate", {
        lan,
        amount: bankForm.mandate_amount,
        max_amount: bankForm.mandate_amount,
        start_date: bankForm.mandate_start_date,
        end_date: bankForm.mandate_end_date,
        frequency: bankForm.mandate_frequency,
        account_no: bankForm.account_no,
        ifsc: bankForm.ifsc,
        account_type: bankForm.account_type,
        customer_name: bankForm.account_holder_name,
        bank_name: bankForm.bank_name,
      });

      if (!mandate.data?.success) {
        setBankError("Mandate failed");
        return;
      }

      setBankResult({ mandate_created: true });
    } catch {
      setBankError("Mandate error");
    } finally {
      setBankLoading(false);
    }
  };
  ...
  const EsignChip = ({ status }) => {
    ...
    return (
      <span style={{ color: map[(status || "PENDING").toUpperCase()] }}>
        {status || "Pending"}
      </span>
    );
  };
  ...
  const columns = [
    {
      ...
      render: (r) => (
        <span
          ...
          onClick={() =>
            navigate(`/approved-loan-details-clayoo/${r.lan}`)
          }
        >
          {r.customer_name}
        </span>
    ...
    },
    {
      key: "status",
      header: "Status",
      render: (r) => <span style={pill(r.status)}>{r.status}</span>,
    },
    {
      key: "limit_entry",
      header: "Final Limit",
      render: (r) => {
        const assigned = r.status === "LIMIT REQUESTED";
        ...
        return (
          <>
            <input
              ...
              disabled={assigned}
              onChange={(e) =>
                handleLimitChange(r.lan, e.target.value)
              }
            />
    ...
    <>
      ...
      {showBankModal && (
        <form onSubmit={handleBankSubmit}>
          <button type="submit">
            {bankLoading ? "Processing..." : "Create Mandate"}
          </button>
        </form>
      )}
    </>
