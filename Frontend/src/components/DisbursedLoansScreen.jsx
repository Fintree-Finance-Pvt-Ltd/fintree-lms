// // components/DisbursedLoansScreen.jsx — server-side paginated
...
//               onClick={() => navigate(`/loan-details/${r.lan}`)} title="View loan details">
//           {r.customer_name ?? "—"}
//         </span>
//       ),
//       sortAccessor: (r) => (r.customer_name || "").toLowerCase(), width: 220,
//     },
//     {
...
//         );
//       },
//       sortAccessor: (r) => (r.status || "").toLowerCase(), width: 140,
//     },
//   ];
...
import DataTable from "./ui/DataTable";
import LoaderOverlay from "./ui/LoaderOverlay";
import "../styles/DisbursedLoans.css"; // Ensure your CSS is imported
...
const DisbursedLoansTable = ({
  ...
  const columns = [
    {
      ...
      render: (r) => (
        <div className="user-info-cell" onClick={() => navigate(`/loan-details/${r.lan}`)}>
          <span className="user-name">{r.customer_name ?? "—"}</span>
          <span className="user-subtext">{r.mobile_number || "No Mobile"}</span>
        </div>
      ),
      sortAccessor: (r) => (r.customer_name || "").toLowerCase(), width: 220,
    },
    {
      ...
      sortAccessor: (r) => (r.lan || "").toLowerCase(), width: 160,
    },
    ...(hasADK ? [{
      key: "batch_id", header: "Batch ID", sortable: true,
      render: (r) => (/^ADK/i.test(r?.lan) ? <span className="id-main">{r.batch_id ?? "—"}</span> : "—"),
      sortAccessor: (r) => (/^ADK/i.test(r?.lan) ? String(r?.batch_id || "").toLowerCase() : ""),
      csvAccessor: (r) => (/^ADK/i.test(r?.lan) ? (r.batch_id ?? "") : ""), width: 140,
    }] : []),
    ...(hasGQFSF ? [{
      key: "app_id", header: "APP ID (FSF)", sortable: true,
      render: (r) => (/^GQFSF/i.test(r?.lan) ? <span className="id-main">{r.app_id ?? "—"}</span> : "—"),
      sortAccessor: (r) => (/^GQFSF/i.test(r?.lan) ? String(r?.app_id || "").toLowerCase() : ""),
      csvAccessor: (r) => (/^GQFSF/i.test(r?.lan) ? (r.app_id ?? "") : ""), width: 140,
    }] : []),
    ...(hasGQNonF ? [{
      key: "app_id", header: "APP ID (Non-FSF)", sortable: true,
      render: (r) => (/^GQNONFSF/i.test(r?.lan) ? <span className="id-main">{r.app_id ?? "—"}</span> : "—"),
      sortAccessor: (r) => (/^GQNONFSF/i.test(r?.lan) ? String(r?.app_id || "").toLowerCase() : ""),
      csvAccessor: (r) => (/^GQNONFSF/i.test(r?.lan) ? (r.app_id ?? "") : ""), width: 140,
    }] : []),
    {
      key: "partner_loan_id", header: "Partner ID", sortable: true,
      render: (r) => <span className="id-main">{r.partner_loan_id ?? "—"}</span>,
      sortAccessor: (r) => String(r?.partner_loan_id || "").toLowerCase(),
      csvAccessor: (r) => r.partner_loan_id ?? "", width: 160,
    },
    {
      key: amountField, header: "Disbursement", sortable: true,
      render: (r) => <span className="currency-text">{formatAmount(r)}</span>,
      sortAccessor: (r) => { const v = Number(r?.[amountField] ?? r?.loan_amount ?? 0); return Number.isFinite(v) ? v : 0; },
      width: 170,
    },
    {
      ...
      render: (r) => {
        ...
        return (
          <span className={`status-pill-modern ${statusClass}`}>
            {r.status ?? "—"}
          </span>
        );
      },
  ...
  return (
    <div className="disbursed-screen">
      <LoaderOverlay show={loading} label="Updating Records..." />
     
      {/* Modern Header Layout */}
      <div className="disbursed-action-bar">
        <div className="title-group">
          <h1>{title}</h1>
          <div className="badge-total">{totalRows.toLocaleString()} Disbursed Records</div>
        </div>
      ...
      <div className="disbursed-table-card">
        ...
        title={title}
          ...
          onPageChange={setPage}
          onPageSizeChange={(n) => setPageSize(n)}
          // Set to null because we moved the search to the modern header above
          renderTopRight={null}
        />
      </div>
    </div>
