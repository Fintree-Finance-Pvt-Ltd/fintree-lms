...
const MotionCorpUpdateData = () => {
  ...
  //       const insurance = res.data?.loan?.insurance_details || {};
  ...
  //         policy_issued_date: insurance.policy_issued_date?.slice(0, 10) || "",
    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        const res = await api.get(
          `/motion-corp/customer-details/${encodeURIComponent(lan)}`,
        );
        ...
        const insuranceAlreadySubmitted = [
        ...
        setInsuranceSubmitted(insuranceAlreadySubmitted);
      } catch (error) {
        console.error("Failed to fetch Motion Corp details:", error);
  ...
  const handleInsuranceSubmit = async (event) => {
    ...
    try {
      ...
      const response = await api.patch(
        `/motion-corp/insurance/${encodeURIComponent(lan)}`,
        insuranceForm,
      );
    ...
    },
    {
      ...
      content: (
        <>
          ...
          >
            <h4 style={{ marginTop: 0 }}>Insurance Details</h4>
            ...
            {insuranceMessage && (
              <div style={{ marginTop: "14px" }}>{insuranceMessage}</div>
            )}
