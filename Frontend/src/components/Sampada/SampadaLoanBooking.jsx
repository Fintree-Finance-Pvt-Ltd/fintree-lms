...
const SampadaLoanBooking = () => {
  ...
  const fetchDealers = async () => {
    try {
      const res = await api.get("sampada/dealersforbooking");
...
`;
  ...
  const handleRunBureauScreening = async () => {
    ...
    try {
      ...
      const res = await api.post("sampada/run-bureau-screening", {
        lan,
      ...
      if (status === "BUREAU APPROVED") {
        setMessage(
          "✅ Sampada bureau screening approved. Moving to the next section.",
        );
      } else if (status === "BUREAU REJECTED") {
        setMessage(
          "⚠️ Sampada bureau screening rejected. Screening is complete, so the case can continue as per the SRBH flow.",
        );
      } else {
        setMessage(
      ...
      setMessage(
        `❌ ${
          ...
          "Sampada bureau screening failed. Please try again."
        }`,
      );
  ...
  const saveBorrowerFirstSection = async () => {
    ...
    try {
      ...
      const res = await api.post("sampada/save-borrower-first-section", {
        ...formData,
  ...
  const verifyOtpHandler = async () => {
    ...
    try {
      ...
      const res = await api.post("sampada/verify-otp", {
        mobile,
        otp,
  ...
  const saveApplicantBeforeAadhaar = async (applicantType) => {
    ...
    await api.post("sampada/save-applicant-details", {
      lan,
      applicantType,
      data: formData,
    });
  ...
  const triggerAadhaar = async (applicantType) => {
    ...
    try {
      ...
      const res = await api.post("sampada/init-aadhaar", {
        lan,
        applicantType,
  ...
  const fetchAndPrefillAadhaarAddress = async (applicantType) => {
    ...
    try {
      ...
      const res = await api.get(
        `sampada/aadhaar-address/${lan}/${applicantType}`,
      );
    ...
    return (
      ...
      >
        <h3 style={{ margin: "0 0 10px" }}>
          {approved ? "✅" : rejected ? "❌" : "⚠️"} Sampada Bureau:{" "}
