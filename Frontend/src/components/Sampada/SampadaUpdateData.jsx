...
// function ExternalLinkField({ label, url, status }) {
...
//   const [retryingApplicant, setRetryingApplicant] = useState("");
...
//       if (!lan) {
//         setErr("LAN is missing. Please open the case from the case list.");
...
//           `/sampada/customer-details/${encodeURIComponent(lan)}`,
//           {
//             params: {
//               _refresh: Date.now(),
//             },
...
//         const insurance = response.data?.loan?.insurance_details || {};

//         setInsuranceForm({
//           insurance_cost: insurance.insurance_cost ?? "",

//           insurance_company_provider:
//             insurance.insurance_company_provider || "",

//           insurance_policy_number: insurance.insurance_policy_number || "",

//           policy_issued_date: insurance.policy_issued_date
//             ? String(insurance.policy_issued_date).slice(0, 10)
//             : "",

//           period_of_insurance: insurance.period_of_insurance || "",
//         });
...
//         setInsuranceSubmitted(alreadySubmitted);
//       } catch (error) {
//         console.error("Failed to fetch Sampada details:", error);
...
//     details?.loan?.verification_status?.borrower,

//     details?.loan?.verification_status?.guarantor,

//     details?.loan?.verification_status?.co_applicant,
//   ].some((applicantStatus) => {
//     if (!applicantStatus) {
//       return false;
//     }

//     const aadhaarStatus = String(applicantStatus.aadhaar_status || "PENDING")
//       .trim()
//       .toUpperCase();

//     const retryCount = Number(applicantStatus.aadhaar_retry_count || 0);

//     const backendCanRetry =
//       applicantStatus.aadhaar_can_retry === true ||
//       Number(applicantStatus.aadhaar_can_retry) === 1;

//     if (aadhaarStatus === "INITIATED") {
//       return true;
//     }

//     return (
//       ["PENDING", "FAILED"].includes(aadhaarStatus) &&
//       Boolean(applicantStatus.aadhaar_initiated_at) &&
//       !backendCanRetry &&
//       retryCount < 2
//     );
//   });
...
//    */
//   const handleAadhaarRetry = async (applicantType) => {
//     if (!lan || retryingApplicant) {
//       return;
//     }
...
//     if (
//       !["BORROWER", "GUARANTOR", "CO_APPLICANT"].includes(
...
//       setDetails(latestResponse.data);
...
//     try {
//       setRetryingApplicant(normalizedApplicantType);

//       const response = await api.post("/sampada/init-aadhaar", {
//         lan,
...
//       console.error("Aadhaar retry failed:", {
//         lan,
...
//       alert(errorPayload.message || "Failed to retry Aadhaar verification.");
//     } finally {
...
//   // const handleInsuranceSubmit = async (event) => {
//   //   event.preventDefault();

//   //   if (!lan) {
//   //     setInsuranceMessage("❌ LAN is missing.");
...
//   //       `/sampada/insurance/${encodeURIComponent(lan)}`,
//   //       insuranceForm,
//   //     );

//   //     setInsuranceSubmitted(true);

//   //     const savedInsurance = response.data?.insurance || insuranceForm;

//   //     setInsuranceForm({
//   //       insurance_cost: savedInsurance.insurance_cost ?? "",

//   //       insurance_company_provider:
//   //         savedInsurance.insurance_company_provider || "",

//   //       insurance_policy_number: savedInsurance.insurance_policy_number || "",

//   //       policy_issued_date: savedInsurance.policy_issued_date
//   //         ? String(savedInsurance.policy_issued_date).slice(0, 10)
//   //         : "",

//   //       period_of_insurance: savedInsurance.period_of_insurance || "",
//   //     });
...
//   //           insurance_details: savedInsurance,
//   //         },
//   //       };
//   //     });
...
//   //     setInsuranceMessage(
//   //       `✅ ${
//   //         response.data?.message || "Insurance details saved successfully"
//   //       }`,
//   //     );
//   //   } catch (error) {
//   //     console.error("Insurance save failed:", error);

//   //     setInsuranceMessage(
//   //       `❌ ${
//   //         error.response?.data?.message || "Failed to save insurance details"
//   //       }`,
//   //     );
//   //   } finally {
...
//             retryingApplicant={retryingApplicant}
//             nowMs={retryClock}
//             onAadhaarRetry={handleAadhaarRetry}
//           />
...
//               url={loan.verification_links?.borrower_aadhaar_url}
//               status={loan.verification_status?.borrower?.aadhaar_status}
//             />

//             <ExternalLinkField
//               label="Guarantor Aadhaar"
//               url={loan.verification_links?.guarantor_aadhaar_url}
//               status={loan.verification_status?.guarantor?.aadhaar_status}
//             />

//             <ExternalLinkField
//               label="Co-applicant Aadhaar"
//               url={loan.verification_links?.co_applicant_aadhaar_url}
//               status={loan.verification_status?.co_applicant?.aadhaar_status}
//             />

//             <ExternalLinkField
//               label="NACH Authentication Link"
//               url={loan.nach_details?.auth_url}
//             />

//             {loan.nach_details?.umrn && (
//               <Field label="UMRN" value={loan.nach_details.umrn} highlight />
//             )}
//           </Grid>
...
//                 min="0"
//                 step="0.01"
//                 value={insuranceForm.insurance_cost}
//                 onChange={handleInsuranceChange}
//                 disabled={insuranceSubmitted || insuranceSaving}
//               />

//               <InsuranceInput
//                 label="Insurance Provider"
//                 name="insurance_company_provider"
//                 value={insuranceForm.insurance_company_provider}
//                 onChange={handleInsuranceChange}
//                 disabled={insuranceSubmitted || insuranceSaving}
//               />
...
//                 type="date"
//                 value={insuranceForm.policy_issued_date}
//                 onChange={handleInsuranceChange}
//                 disabled={insuranceSubmitted || insuranceSaving}
//               />
...
//                 value={insuranceForm.period_of_insurance}
//                 onChange={handleInsuranceChange}
//                 disabled={insuranceSubmitted || insuranceSaving}
//               />
//             </Grid>
...
//             <button
//               type="submit"
//               disabled={insuranceSaving || insuranceSubmitted}
//               style={{
//                 marginTop: "18px",
//                 padding: "11px 22px",
//                 border: 0,
...
//                 cursor:
//                   insuranceSaving || insuranceSubmitted
//                     ? "not-allowed"
//                     : "pointer",

//                 opacity: insuranceSaving ? 0.75 : 1,
//               }}
//             >
//               {insuranceSubmitted
...
//   const aadhaarStatus = String(status.aadhaar_status || "PENDING")
//     .trim()
//     .toUpperCase();

//   const retryCount = Number(status.aadhaar_retry_count || 0);
...
//    */
//   const backendCanRetry =
//     status.aadhaar_can_retry === true || Number(status.aadhaar_can_retry) === 1;
...
//     const normalizedTime = new Date(String(value).replace(" ", "T")).getTime();

//     return Number.isFinite(normalizedTime) ? normalizedTime : null;
//   };

//   const initiatedAtMs = parseDatabaseDate(status.aadhaar_initiated_at);

//   const backendRetryAvailableAtMs = parseDatabaseDate(
//     status.retry_available_at,
//   );

//   const retryAvailableAtMs =
//     backendRetryAvailableAtMs ||
//     (initiatedAtMs ? initiatedAtMs + 24 * 60 * 60 * 1000 : null);

//   const currentTime = Number(nowMs || Date.now());

//   const remainingMilliseconds = retryAvailableAtMs
//     ? Math.max(0, retryAvailableAtMs - currentTime)
//     : Math.max(
//         0,

//         Number(status.retry_seconds_remaining || 0) * 1000,
//       );

//   /*
//    * VERIFIED is intentionally excluded.
//    */
//   const retryableStatus = ["PENDING", "FAILED", "INITIATED"].includes(
//     aadhaarStatus,
//   );
...
//     retryableStatus &&
//     !retryLimitReached &&
//     (isFirstPendingAttempt || backendCanRetry);
...
//   const isRetrying = retryingApplicant === applicantType;

//   /*
//    * VERIFIED means no button rendered.
//    */
//   const showRetryButton = retryableStatus;

//   const retryButtonDisabled =
//     isRetrying || retryLimitReached || !canRetryAadhaar;
...
//     if (remainingHours <= 0) {
//       return `${Math.max(1, remainingMinutes)} minute(s)`;
//     }
...
//     if (retryWaitActive) {
//       return "Retry available in " + formatRemainingTime();
//     }
...
//     return `Retry Aadhaar ` + `(${retriesRemaining} left)`;
//   })();
...
//         <div>
//           <VerificationField label="Aadhaar Status" value={aadhaarStatus} />

//           {showRetryButton && (
//             <button
//               type="button"
//               disabled={retryButtonDisabled}
//               onClick={() => onAadhaarRetry(applicantType)}
//               style={{
//                 marginTop: "12px",
//                 padding: "9px 16px",

//                 border: `1px solid ${
//                   retryButtonDisabled ? "#cbd5e1" : "#2563eb"
//                 }`,

//                 borderRadius: "8px",

//                 background: retryButtonDisabled ? "#e2e8f0" : "#ffffff",

//                 color: retryButtonDisabled ? "#64748b" : "#1d4ed8",

//                 fontSize: "13px",
//                 fontWeight: 800,

//                 cursor: retryButtonDisabled ? "not-allowed" : "pointer",

//                 opacity: retryButtonDisabled ? 0.8 : 1,
//               }}
//             >
//               {retryButtonText}
//             </button>
...
//               }}
//             >
//               Aadhaar retry is locked for 24 hours from the previous attempt.{" "}
//               {formatRemainingTime()} remaining.
//             </div>
//           )}

//           {retryLimitReached && retryableStatus && (
//             <div
//               style={{
//                 marginTop: "10px",
//                 color: "#991b1b",
...
//               }}
//             >
//               Maximum two Aadhaar retries have been used.
//             </div>
//           )}
//         </div>
...
// export default SampadaUpdateData;
...
/*
 * This page never opens customer/provider links.
 * It only lets the operator copy them.
 */
function CopyLinkField({ label, url, status }) {
  const [copied, setCopied] = useState(false);
        ...
        <>
          ...
          <button
            ...
            style={{
              width: "fit-content",
              padding: "8px 14px",
              border: `1px solid ${copied ? "#86efac" : "#2563eb"}`,
              borderRadius: "8px",
              background: copied ? "#dcfce7" : "#ffffff",
...
const findAgreementSignatory = ({ signatories, mobile, email, name }) => {
  ...
  return (
    signatories.find(
      (item) =>
        targetMobile && normalizeMobile(item?.contact_number) === targetMobile,
    ) ||
    signatories.find(
      (item) => targetEmail && normalizeText(item?.email) === targetEmail,
    ) ||
    signatories.find(
      (item) => targetName && normalizeText(item?.name) === targetName,
    ) ||
    null
  );
};

const getAgreementUrl = (agreementEntry) =>
  agreementEntry?.url ||
  agreementEntry?.doqfy_sign_url ||
  agreementEntry?.sign_url ||
  agreementEntry?.agreement_url ||
  agreementEntry?.esign_url ||
  null;

const getAgreementStatus = (agreementEntry) =>
  agreementEntry?.status || agreementEntry?.esign_status || null;
...
const SampadaUpdateData = () => {
  ...
  /*
   * NACH is loan-level in the current Sampada response.
   * Support both old/new response key names.
   */
  const enachUrl =
    loan.nach_details?.auth_url ||
    loan.enach_details?.auth_url ||
    loan.verification_links?.nach_url ||
    loan.verification_links?.enach_url ||
    null;

  const enachStatus =
    loan.nach_details?.status || loan.enach_details?.status || null;

  /*
   * Agreement may be returned either already normalized per applicant,
   * or as the raw Doqfy response containing webhook_response.signatory_data.
   */
  const agreementDetails = loan.agreement_details || {};

  const agreementRawResponse =
    agreementDetails.raw_response ||
    agreementDetails.response ||
    loan.agreement_response ||
    details.agreement_response ||
    details.agreement ||
    agreementDetails;

  const agreementSignatories =
    agreementRawResponse?.webhook_response?.signatory_data ||
    agreementRawResponse?.signatory_data ||
    [];

  const borrowerAgreementFromRaw = findAgreementSignatory({
    signatories: agreementSignatories,
    mobile: loan.mobile_number,
    email: loan.email,
    name: loan.customer_name,
  });

  const guarantorAgreementFromRaw = findAgreementSignatory({
    signatories: agreementSignatories,
  ...
  const coApplicantAgreementFromRaw = findAgreementSignatory({
    signatories: agreementSignatories,
    mobile: loan.co_applicant?.mobile,
    email: loan.co_applicant?.email,
    name: loan.co_applicant?.name,
  });

  const borrowerAgreement =
    agreementDetails.borrower || borrowerAgreementFromRaw;

  const guarantorAgreement =
    agreementDetails.guarantor || guarantorAgreementFromRaw;

  const coApplicantAgreement =
    agreementDetails.co_applicant ||
    agreementDetails.coApplicant ||
    coApplicantAgreementFromRaw;
  ...
  const sections = [
      ...
      content: (
        ...
        >
          <ApplicantVerificationBlock
            title="Borrower"
            applicantType="BORROWER"
            aadhaarUrl={loan.verification_links?.borrower_aadhaar_url}
            data={loan.verification_status?.borrower}
            retryingApplicant={retryingApplicant}
            nowMs={retryClock}
            onAadhaarRetry={handleAadhaarRetry}
          ...
          {hasValue(loan.guarantor?.name) && (
            <ApplicantVerificationBlock
              title="Guarantor"
              applicantType="GUARANTOR"
              aadhaarUrl={loan.verification_links?.guarantor_aadhaar_url}
              data={loan.verification_status?.guarantor}
              retryingApplicant={retryingApplicant}
              nowMs={retryClock}
              onAadhaarRetry={handleAadhaarRetry}
          ...
          {hasValue(loan.co_applicant?.name) && (
            <ApplicantVerificationBlock
              title="Co-Applicant"
              applicantType="CO_APPLICANT"
              aadhaarUrl={loan.verification_links?.co_applicant_aadhaar_url}
              data={loan.verification_status?.co_applicant}
              retryingApplicant={retryingApplicant}
              nowMs={retryClock}
              onAadhaarRetry={handleAadhaarRetry}
    ...
    {
      title: "eNACH, Agreement & Insurance",
      icon: "🛡️",
      content: (
        <>
          <div>
            ...
            <Grid>
              <CopyLinkField
                label="eNACH Authentication Link"
                url={enachUrl}
                status={enachStatus}
              />
          ...
          >
            ...
            <Grid>
              <Field
                label="Insurance Cost"
                value={loan.insurance_details?.insurance_cost}
              />

              <Field
                label="Insurance Provider"
                value={loan.insurance_details?.insurance_company_provider}
              />

              <Field
                label="Policy Number"
                value={loan.insurance_details?.insurance_policy_number}
              />

              <Field
                label="Policy Issued Date"
                value={formatDate(loan.insurance_details?.policy_issued_date)}
              />

              <Field
                label="Period of Insurance"
                value={loan.insurance_details?.period_of_insurance}
              />
            </Grid>
          </div>
        </>
