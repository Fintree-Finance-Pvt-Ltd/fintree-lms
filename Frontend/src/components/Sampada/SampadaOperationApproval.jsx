...
const SampadaOperationApproval = ({
  apiUrl = "/sampada/operation-initiated-loans",
  title = "Sampada Operation Approval Loans",
}) => {
  ...
  render: (r) => {
    const bankStatus = (r.bank_status || "").toUpperCase();

    // ONLY CONDITION FOR NOW
    ...
    const handleApprove = async () => {
      ...
      try {
        await api.post(`/sampada/${r.lan}/approve`);

        setToast({
          type: "success",
          msg: `Loan ${r.lan} approved successfully.`,
        });
    ...
    const handleReject = async () => {
      ...
      try {
        await api.post(`/sampada/${r.lan}/reject`, {
          reason,
        });

        setToast({
          type: "success",
          msg: `Loan ${r.lan} rejected.`,
        });
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            style={{
              padding: "8px 14px",
              borderRadius: 10,
              border: canApproveReject
                ? "1px solid #86efac"
                : "1px solid #d1d5db",
              background: canApproveReject
                ? "#ecfdf3"
                : "#f9fafb",
              color: canApproveReject
                ? "#166534"
                : "#9ca3af",
              cursor: canApproveReject
                ? "pointer"
                : "not-allowed",
              fontWeight: 700,
              fontSize: 12,
...
export default SampadaOperationApproval;
