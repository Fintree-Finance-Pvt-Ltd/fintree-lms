...
const SampadaDealerLoginActions = () => {
  ...
  */
  useEffect(() => {
    ...
    api
      .get("/sampada/dealers-login-cases")
      .then((res) => {
  ...
  */
  const handleStatusChange = async (lan, status) => {
    try {
      ...
      await api.patch(`/sampada/dealer/status/${lan}`, {
...

    {
      ...
      render: (r) => (
        <span
          ...
          onClick={() => navigate(`/sampada/dealer-details/${r.lan}`)}
        >
          {r.business_name}
        </span>
      ),
  ...
  */
  return (
    <DataTable
      title="Sampada Dealer Credit Approval"
      rows={rows}
      columns={columns}
      globalSearchKeys={[
        "business_name",
      ...
      exportFileName="sampada_dealers"
    />
  );
};

export default SampadaDealerLoginActions;