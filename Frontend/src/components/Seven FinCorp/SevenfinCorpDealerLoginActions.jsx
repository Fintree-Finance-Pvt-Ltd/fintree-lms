...
const SevenFinCorpDealerLoginActions = () => {
  ...
  */
  useEffect(() => {
    ...
    api
      .get("/seven-fincorp/dealers-login-cases")
      .then((res) => {
  ...
  const columns = [
    {
      ...
      render: (r) => (
        <span
          ...
          onClick={() => navigate(`/seven-fincorp/dealer-details/${r.lan}`)}
        >
          {r.business_name}
        </span>
      ),
  ...
  */
  return (
    <DataTable
      title=" Seven FinCorp Dealer Credit Approval"
      rows={rows}
      columns={columns}
      globalSearchKeys={[
        "business_name",
      ...
      exportFileName="seven_fincorp_dealers"
    />
  );
};

export default SevenFinCorpDealerLoginActions;
