...
const CustomerListScreen = ({
  ...
  const partnerCounts = useMemo(() => {
    ...
    PARTNERS.forEach((p) => {
      counts[p.prefix] = rows.filter(
        (r) => r.lan?.startsWith(p.prefix)).length;
    });
    return counts;
  }, [rows]);
  ...
  const columns = [
    {
      ...
      render: (r) => (
        <div className="cust-info" onClick={() => navigate(`/customers/${r.partner_loan_id}`)}>
          <span className="cust-name">{r.applicant_name ?? "—"}</span>
          <span className="cust-id">LAN: {r.lan}</span>
        </div>
      ),
      width: 240,
    },
    {
      ...
      render: (r) => {
        const sanctioned = Number(r.sanction_amount || 0);
        const utilized = Number(r.utilized_sanction_limit || 0);
        const percent = sanctioned > 0 ? (utilized / sanctioned) * 100 : 0;
        return (
          <div className="limit-stack">
            <div className="limit-vals">
              <span className="utilized">₹{utilized.toLocaleString("en-IN")}</span>
              <span className="total"> / ₹{sanctioned.toLocaleString("en-IN")}</span>
            </div>
    ...
    },
    {
      key: "partner_loan_id",
      header: "Partner Ref",
      render: (r) => <span className="ref-badge">{r.partner_loan_id ?? "—"}</span>,
      width: 160,
    },
    {
      key: "sanction_amount",
...

    {
      ...
      render: (r) => (
        <div className="btn-group-modern">
          <button className="btn-ghost" onClick={() => navigate(`/customers/${r.lan}/invoices`)}>Invoices</button>
          <button className="btn-ghost" onClick={() => navigate(`/customers/${r.partner_loan_id}/suppliers`)}>Suppliers</button>
          <button className="btn-ghost" onClick={() => navigate(`/customers/${r.lan}/repayments`)}>Repayments</button>
          <button className="btn-primary-sm" 
          onClick={() => navigate(`/customers/${r.lan}/allocation`)}>Allocation</button>
        </div>
      ),
      width: 440,
    },
  ...
  return (
    <div className="customer-list-container">
      <LoaderOverlay show={loading} label="Loading customers..." />

      <div className="cust-header-modern">
        <div className="cust-title-area">
          <h1>{title}</h1>
          <p>Manage and monitor partner-wise supply chain limits</p>
        </div>
        ...
        <div className="partner-segments">
          {PARTNERS.map((p) => (
            <button
              key={p.prefix}
              className={`segment-btn ${selectedPartner === p.prefix ? "active" : ""}`}
              onClick={() => setSelectedPartner(p.prefix)}
            >
              {p.label}
              <span className="segment-count">{partnerCounts[p.prefix] || 0}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="table-card-modern">
        {err && <div className="error-alert-inline">{err}</div>}
        <DataTable
 title={`${title} — ${
          PARTNERS.find(p => p.prefix === selectedPartner)?.label
        }`}     
     ...
     globalSearchKeys={[
          ...
          "lan",
                  ...
                  initialSort={{ key: "applicant_name", dir: "asc" }}
          exportFileName="customer_limit_report"
               initialPageSize={pageSize}
        pageSizeOptions={[10, 25, 50, 100]}
          serverPagination={true}
          totalRows={filteredRows.length}
